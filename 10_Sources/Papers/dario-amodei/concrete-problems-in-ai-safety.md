---
type: source
title: Concrete Problems in AI Safety
authors:
- Dario Amodei
- Chris Olah
- Jacob Steinhardt
- Paul Christiano
- John Schulman
- Dan Mane
thinker:
- '[[20_People/dario-amodei/profile|Dario Amodei]]'
- '[[20_People/chris-olah/profile|Chris Olah]]'
- '[[20_People/paul-christiano/profile|Paul Christiano]]'
source_type: paper
venue: arXiv
year: 2016
url: https://arxiv.org/abs/1606.06565
domain:
- alignment
- safety
status: review
created: 2026-04-22
tags:
- paper
- foundational
- alignment-agenda
nlm_source_id: 68d5b53f-b9b6-4329-a373-fe3b65c0e433
topic:
- topic/ai-safety
- topic/alignment
subject:
- subject/dario-amodei
- subject/chris-olah
- subject/paul-christiano
- subject/google-brain
- subject/concrete-problems-in-ai-safety
wiki_indexed: '2026-04-27T07:27:46Z'
wiki_hash: 7ebdcaa50cc10c3008e77c3386442b4bf59420790bec41c2f75783025a4b681f
wiki_role: raw
---


# Concrete Problems in AI Safety (2016)

## Citation

Amodei, D., Olah, C., Steinhardt, J., Christiano, P., Schulman, J., & Mane, D. (2016). *Concrete Problems in AI Safety*. arXiv:1606.06565.

## One-line summary

Foundational alignment research agenda identifying five concrete problems that persist under capability scaling: negative side effects, reward hacking, scalable oversight, safe exploration, and robustness to distributional shift.

## Key claims

- Safety research can be framed as engineering problems on present-day systems, not only as long-run speculation.
- Five problems: (1) avoiding negative side effects, (2) avoiding reward hacking, (3) scalable oversight under expensive human feedback, (4) safe exploration in live environments, (5) robustness to distributional shift.
- Each problem admits multiple research directions pursuable with current techniques.
- Agenda positions alignment as coupled to capability work rather than a separate discipline.

## Excerpts

Not yet extracted. Extraction pending — notebook did not cite this source in the five position queries. Candidate for a follow-up NotebookLM query on the foundational alignment agenda.

## Critical evaluation

The 2016 framing treats alignment as an open research agenda. By 2024, Amodei publicly frames alignment as "tractable with effort" and non-binary. Trajectory from 2016 to 2024 is a key evolution worth tracking in `positions.md`.

## Related

- [[20_People/dario-amodei/profile|Dario Amodei]]
- [[20_People/chris-olah/profile|Chris Olah]]
- [[20_People/paul-christiano/profile|Paul Christiano]]
- [[10_Sources/Articles/dario-amodei/machines-of-loving-grace|Machines of Loving Grace]]
